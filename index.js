...
async function fetchDataAndStore() {
  try {
    ...
    const res = await Promise.all([
      ...
      fetch(
        `https://api.finary.com/users/me/portfolio/timeseries?sharing_link_id=e9c52c1f0391ee54bc4f&period=all&timeseries_type=sum&value_type=gross&new_format=true`
      ),
      //   fetch(
      //     `https://euw1.api.riotgames.com/lol/league/v4/entries/by-summoner/dDH5QL0rPpppwvw4QSv2VYpm-9-b__A1sjpITII06KnE8vw?api_key=${process.env.RIOT_API_KEY}`
      //   ),
    ]);
    ...
    const values = {
      ...
      money:
        data[2].result[0].timeseries[
          data[2].result[0].timeseries.length - 1
        ][1],
      lol: 1994,
      date: new Date(),
    };
